...
```{r}
# Figure S2: Barcode density graphs and replicate correlations
# Histograms
...
ggsave('../FigureSX/FigureS2.pdf', height = 16, width = 17)
rm(h1, c1a, c1b, h3, c3a, c3b, h2, c2a, c2b, h4, c4a, c4b)

# save data table as input
write.table(induce_exp, '../source_data/FigureS2.txt', quote = F, row.names = F, sep = '\t')

nrow(induce_combo) # 1493
nrow(induce_multiple) # 1638
nrow(induce_distal) # 3769
nrow(induce_steric) # 1369
```
...
# save data table as input
write.table(induce_combo, '../source_data/FigureS3.txt', quote = F, row.names = F, sep = '\t')
```

```{r}
# Figure S5: Training Pcombo thermodynamic model on unseen sequenes
CV_results = t(read.csv('../source_data/FigureS5A.csv', header = FALSE, row.names = 1)) %>% as.data.frame() %>% select(-`Training Fraction`) %>% gather(Proportion, R2, 1:8)
CV_results %>% filter(R2 != "Indeterminate") %>% ggplot(aes(x = as.factor(Proportion), y = as.numeric(R2))) +
  geom_boxplot(fill = "#3C9CB4", alpha = 0.6, outlier.shape = NA) + geom_jitter(pch = 21, fill = "#3C9CB4", alpha = 0.9) +
  ylim(0, 1) + labs(x = 'Proportion of Data Trained On', y = 'R2 of Predicted vs. Actual Expression')
ggsave('../FigureSX/FigureS5A_jitter.pdf', height = 5, width = 6.5)
```
...
```{r}
# Figure S10: Plate reader supplement with titration profiles of selected promoters
...
```{r}
# calculating R2 for Figures 3A and S5B
...
(cor.test(as.numeric(test$Actual), as.numeric(test$Predicted), method = "pearson")$estimate)^2 
# R2 = 0.79

data = as.data.frame(read.csv('../source_data/FigureS5B.csv', header = FALSE)) %>%
  gather(Expression, Values)
data$Values = gsub("\\{", "", data$Values)
data$Values = gsub("\\}", "", data$Values)
test = data %>% separate(col = "Values", into = c("Predicted", "Actual"), sep = ", ")
test$Predicted = as.numeric(test$Predicted)
test$Actual = as.numeric(test$Actual)
test %>% ggplot(aes(x = log10(Predicted), y = log10(Actual))) + geom_point(aes(color = Expression))

(cor.test(log10(test$Predicted), log10(test$Actual), method = "pearson")$estimate)^2 
# R2 = 0.73
```