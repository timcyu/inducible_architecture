...
```{r}
#TF_spacing <- subset(loop_vars, !grepl("apFAB", loop_vars$name))
#read.table("../")
...
#Figure 1D - Repression by proximal site
...
left_join(mean_proximal, mean_noproximal, by = 'TF') %>%
    ...
    labs(x = 'Transcription Factor', y = 'Relative repression by proximal site', Title = 'Effect of Repressor at Proximal Site') + 
...
ggsave('../Figure1/Figure1D.pdf', units = 'in', width = 14, height = 6)
```

Figure 1E - Repression by lone Distal site
```{r}

#temp <- loop_vars %>% filter(., !grepl("apFAB",name), Background == 'ind', Proximal == 'nullproximal')
...
#Figure 1F - repression as a function of operator spacing
```{r}

#Normalize by dividing distal by nulldistal

nulldistal_exp <- filter(TF_spacing,Distal == 'nulldistal', Background == 'ind')[c(12,14,6)] %>% mutate(nulldistal_exp=RNA_exp_12) %>% select(-RNA_exp_12)

TF_spacing %>% filter(., Proximal == 'proximal', Background == 'ind', Distal == 'distal') %>% 
  mutate(Phase = Offset/11, spacing=Offset+95) %>% 
  ...
  facet_grid(~TF) +
  ylim(.6,1.25) +
  labs(title = 'Expression Normalized by Null Sequence', y = 'Relative expression with distal', x = 'Distance between operators (bp)') +
          theme(axis.text.x = element_text(size = 20, angle = 45, hjust = 1), 
                axis.title.x = element_text(size = 20),
                axis.text.y = element_text(size = 20), 
                axis.title.y = element_text(size = 20), 
